...
export function AdminLayout() {
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-[calc(100vh-4rem)]">
        <aside className="hidden lg:block w-64 border-r bg-slate-50 shrink-0 p-4">
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)]">
      {/* ── Sidebar ── */}
      <aside className="hidden lg:flex lg:flex-col w-64 border-r bg-slate-50 shrink-0">
        {/* Profile area — clickable */}
        <div className="p-4 border-b relative" ref={menuRef}>
          <p className="text-xs text-[var(--muted)] uppercase tracking-wider">Admin Panel</p>
          <button
            onClick={() => setMenuOpen(!menuOpen)}
            className="mt-1 flex items-center gap-1.5 text-sm font-medium hover:text-slate-700 transition-colors w-full text-left group"
          >
            {/* Avatar circle */}
            {user?.avatar ? (
              <img
              ...
              <span className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-semibold text-slate-600 shrink-0">
          ...
          {/* Dropdown menu */}
          {menuOpen && (
            <div className="absolute left-0 right-0 top-full mt-1 mx-4 bg-white border rounded-lg shadow-lg z-50 overflow-hidden">
              <button
                onClick={() => {
                  setMenuOpen(false)
          ...
          {navItems.slice(0, 5).map(({ to, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) => cn(
                ...
                isActive ? 'text-slate-900 font-medium' : 'text-[var(--muted)]'
              )}
            >
              <Icon size={18} />
      ...
      {/* ── Main content ── */}
      <main className="flex-1 p-6 pb-20 lg:pb-6 bg-slate-50/50">
        <Outlet />
      </main>
    </div>
  )
