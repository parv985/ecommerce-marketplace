...
# seller approves + completes
APPROVED=$(curl -s -X PATCH "$BASE/returns/$RETURN_ID/status" -H "Authorization: Bearer $SELLER_TOKEN" \
  -H "Content-Type: application/json" -d '{"status":"APPROVED"}' | json 'd["data"]["status"]')
check "seller approves" "APPROVED" "$APPROVED"
...
check "stock restored on completion (2->3)" "3" "$STOCK_RESTORED"
