...
> This is the backend application of the `ecommerce-marketplace` monorepo and lives in the `backend/` folder, alongside the React client in `frontend/`. All commands below are run from this `backend/` folder.
...
For comprehensive API testing instructions, see the [Postman Testing Guide](docs/POSTMAN_COMPLETE_TESTING_GUIDE.md).
